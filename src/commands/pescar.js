...
module.exports = {
  ...
  async run(ctx, isSlash) {
    ...
    const username = isSlash ? ctx.user.username : ctx.author.username;
    ...
    db.get(
      ...
      (err, item) => {
        ...
        if (err) {
          console.error(err);
          return ctx.reply("❌ Erro no banco de dados.");
        }
        ...
        db.get(
          ...
          (err, user) => {
            ...
            const lastUsed = user[`last_${jobType}`] || 0;
            ...
            db.run(
              ...
              async (err) => {
                ...
                if (err) {
                  console.error(err);
                  return ctx.reply("❌ Erro ao salvar dados.");
                }
